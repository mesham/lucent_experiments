...
static void execute_on_device(cl::Event & copyOnEvent, cl::Event & kernelExecutionEvent, cl::Event & copyOffEvent) {
  ...
  OCL_CHECK(err, err = command_queue->enqueueTask(*axpy_kernel, &kernel_wait_events, &kernelExecutionEvent));
...
static void init_device(char * binary_filename, int data_size) {
  ...
  OCL_CHECK(err, axpy_kernel=new cl::Kernel(*program, "axpy_kernel", &err));
  ...
  OCL_CHECK(err, err = axpy_kernel->setArg(0, *buffer_x));
  OCL_CHECK(err, err = axpy_kernel->setArg(1, *buffer_y));  
  OCL_CHECK(err, err = axpy_kernel->setArg(2, *buffer_result));
  OCL_CHECK(err, err = axpy_kernel->setArg(3, data_size));
}
