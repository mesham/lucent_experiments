...
DATA_TYPE *x_data, *result_data; // Input and result data

cl::CommandQueue * command_queue;
cl::Context * context;
cl::Program * program;
cl::Kernel * l2norm_kernel;
cl::Buffer *buffer_x, *buffer_result; // Buffers to transfer to and from the device
...
static void execute_on_device(cl::Event & copyOnEvent, cl::Event & kernelExecutionEvent, cl::Event & copyOffEvent) {
  ...
  OCL_CHECK(err, err = command_queue->enqueueMigrateMemObjects({*buffer_x}, 0, nullptr, &copyOnEvent));	

  // Queue kernel execution
  std::vector<cl::Event> kernel_wait_events;
  kernel_wait_events.push_back(copyOnEvent);
  OCL_CHECK(err, err = command_queue->enqueueTask(*l2norm_kernel, &kernel_wait_events, &kernelExecutionEvent));
...
static void init_device(char * binary_filename, int data_size) {
  ...
  OCL_CHECK(err, l2norm_kernel=new cl::Kernel(*program, "l2norm_kernel", &err));
  ...
  OCL_CHECK(err, err = l2norm_kernel->setArg(0, *buffer_x));
  OCL_CHECK(err, err = l2norm_kernel->setArg(1, *buffer_result));
  OCL_CHECK(err, err = l2norm_kernel->setArg(2, data_size));
}
