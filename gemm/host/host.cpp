...
static void init_device(char*, int, int, int);
static void execute_on_device(cl::Event&,cl::Event&,cl::Event&);
static void init_problem(int, int, int);
...
int main(int argc, char * argv[]) {    
  ...
  int data_size_k=atoi(argv[4]);

  init_problem(data_size_m, data_size_n, data_size_k);
  init_device(argv[1], data_size_m, data_size_n, data_size_k);
  ...
  printf("Total runtime : %f ms, (%f ms xfer on, %f ms execute, %f ms xfer off) for %d by %d by %d elements\n", 
        copyOnTime+kernelTime+copyOffTime, copyOnTime, kernelTime, copyOffTime, data_size_m, data_size_n, data_size_k);    
...
*/
static void init_device(char * binary_filename, int data_size_m, int data_size_n, int data_size_k) {
  cl_int err;
  ...
  OCL_CHECK(err, buffer_x=new cl::Buffer(*context, CL_MEM_USE_HOST_PTR  | CL_MEM_READ_ONLY, data_size_m * data_size_k * sizeof(DATA_TYPE), x_data, &err));
  OCL_CHECK(err, buffer_y=new cl::Buffer(*context, CL_MEM_USE_HOST_PTR  | CL_MEM_READ_ONLY, data_size_n * data_size_k * sizeof(DATA_TYPE), y_data, &err));
  OCL_CHECK(err, buffer_result=new cl::Buffer(*context, CL_MEM_USE_HOST_PTR  | CL_MEM_WRITE_ONLY, data_size_n * data_size_m * sizeof(DATA_TYPE), result_data, &err));

  // Set kernel arguments
  OCL_CHECK(err, err = gemm_kernel->setArg(0, *buffer_x));
  OCL_CHECK(err, err = gemm_kernel->setArg(1, *buffer_y));  
  OCL_CHECK(err, err = gemm_kernel->setArg(2, *buffer_result));
  OCL_CHECK(err, err = gemm_kernel->setArg(3, data_size_m));
  OCL_CHECK(err, err = gemm_kernel->setArg(4, data_size_n));
  OCL_CHECK(err, err = gemm_kernel->setArg(5, data_size_k));
}
...
*/
static void init_problem(int data_size_m, int data_size_n, int data_size_k) {
  x_data=(DATA_TYPE*) memalign(PAGESIZE, sizeof(DATA_TYPE) * data_size_m * data_size_k);
  y_data=(DATA_TYPE*) memalign(PAGESIZE, sizeof(DATA_TYPE) * data_size_n * data_size_k);
  result_data=(DATA_TYPE*) memalign(PAGESIZE, sizeof(DATA_TYPE) * data_size_n * data_size_m);
  for (int i=0;i<data_size_m * data_size_k;i++) {
    x_data[i]=(DATA_TYPE) i;
  }
  for (int i=0;i<data_size_n * data_size_k;i++) {
